...
def experiment_pretty_print(
    ...
    """Log a human-readable summary of a single VQE run."""
    it_per_sec: float = iterations / execution_time if execution_time > 0 else 0.0
...
def run_single_experiment(
    ...
    """
    Run a single experiment given an experiment_config dictionary.
    ...
    try:
        ...
        pipeline = VQEPipeline.from_config(
            current_mol, current_ansatz, current_opt, current_noise
        )

        n_qubits: int = pipeline.n_qubits
        n_params: int = pipeline.ansatz.get_num_params()

        iterations, history, optimal_params, converged, execution_time = pipeline.run()
        energies = history["energies"]
        final_energy = energies[-1]

        experiment_pretty_print(
            converged,
            final_energy,
            iterations,
            execution_time,
        ...
        return None, f"Error {str(e)} in running: {run_id}"
